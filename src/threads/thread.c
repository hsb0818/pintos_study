...
void thread_sleep(const int64_t wakeup_tick)
{
    enum intr_level old_level = intr_disable ();
    struct thread* t = thread_current();
    ...
    list_insert_ordered(&sleep_list, &t->elem, thread_awake_priority_comp, NULL);
...
void thread_awake(const int64_t current_tick)
{
...
bool thread_awake_priority_comp(const struct list_elem *a, const struct list_elem *b, void *aux)
{
...
void donate_priority(void)
{
    ...
    while (lock != NULL)
    {
        if (lock->holder == NULL)
            break;
        else if (cur->priority <= lock->holder->priority)
            break;
...
/*
 * Remove all the donation thread that blocked by this lock among the 
 * current thread's donation lists.
*/
...
{
    struct thread *cur = thread_current();
    int donate_priority = PRI_NONE;

    if (!list_empty(&cur->donations))
        donate_priority = list_entry(list_front(&cur->donations), struct thread, donate_elem)->priority;

    cur->priority = (cur->init_priority < donate_priority) ? donate_priority : cur->init_priority;
}

void test_priority_and_yield(void)
{
    if (list_empty(&ready_list))
        return;
